...
// Project a point onto an another point (called the eye)
My2DPoint projectPoint(My3DPoint eye, My3DPoint p) {
  float normalize = 1 - p.z / (float)eye.z;
  return new My2DPoint((p.x - eye.x)/normalize, (p.y - eye.y)/normalize);
}

//A 2D box that is the projection of a 3D box
class My2DBox {
  My2DPoint[] s;
  ...
  //render the box in 3 color : green, blue, red
  void render(){
    strokeWeight(3);
...
// Project a 3D box as a 2D box seen by the point "eye"
My2DBox projectBox (My3DPoint eye, My3DBox box) {
  My2DPoint[] r = new My2DPoint[box.p.length];
  for(int i = 0; i < box.p.length; i++){
    r[i] = projectPoint(eye, box.p[i]);
  }
  return new My2DBox(r);
}
